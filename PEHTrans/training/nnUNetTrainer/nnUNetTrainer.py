...
class nnUNetTrainer(object):
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        self.print_to_log_file(f"Setting deep supervision to {enabled}", also_print_to_console=True)
        
        if isinstance(mod, TokenSeg):
            self.print_to_log_file("PEHTrans model detected. Deep supervision is handled by the model's forward pass.", also_print_to_console=True)
            return
        elif hasattr(mod, 'decoder_output3') and hasattr(mod, 'decoder_output2') and hasattr(mod, 'decoder_output1')and hasattr(mod, 'decoder_output4'):
            mod.decoder.deep_supervision = enabled
            self.print_to_log_file(f"Deep supervision enabled in decoder: {enabled}", also_print_to_console=True)
        else:
            self.print_to_log_file("Warning: The model does not support deep supervision. Depth supervision is not enabled.", also_print_to_console=True)
